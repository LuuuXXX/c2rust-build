...
fn track_with_wrapper(
    ...
    // Extract compilers from the log before processing
    let compilers = extract_compilers_from_log(&log_file)?;
...
fn convert_log_to_json(log_file: &Path, output_file: &Path) -> Result<()> {
    ...
    for line in reader.lines() {
        ...
        if line.starts_with("DIR:") {
            ...
            // Skip compiler line for now, handled in extract_compilers_from_log
            continue;
...
fn extract_compilers_from_log(log_file: &Path) -> Result<Vec<String>> {
    if !log_file.exists() {
        return Ok(Vec::new());
    }
