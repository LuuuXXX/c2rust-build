...
4. **构建追踪**：使用 LD_PRELOAD 钩子库在追踪编译器调用的同时执行构建命令
   ...
   - 拦截链接器调用（ld/lld），提取构建的二进制目标
   - **新的 libhook.so 直接生成预处理文件**到 `.c2rust/<feature>/c/` 目录
   - **自动记录构建目标**到 `.c2rust/<feature>/c/targets.list` 文件
5. **文件收集与选择**：
...
└── .c2rust/
    ...
    └── <feature>/                  # "default" 或指定的特性
        ...
        │   ├── targets.list        # 构建目标列表（静态库、动态库、可执行文件）
        │   └── src/                # 保留源目录结构
        │       ├── module1/
...
4. **构建目标追踪**：拦截链接器调用（ld/lld），自动提取构建的二进制文件
5. **信息记录**：记录编译选项、文件路径和工作目录
6. **输出格式**：使用 `---ENTRY---` 分隔符格式化输出
7. **线程安全**：使用文件锁处理并行构建
...
Hook 库会自动追踪构建过程中生成的所有二进制文件，并将它们写入到 `$(C2RUST_FEATURE_ROOT)/c/targets.list` 文件中。

**追踪的二进制类型：**
- **静态库** (`.a` 文件)：仅记录项目根目录下的静态库，且需要作为完整文件路径传递给链接器（例如 `libmath.a` 或 `./libmath.a`）。注意：通过 `-l` 标志指定的库（例如 `-lmath`）不会被记录。
- **动态库** (`.so` 文件)：记录所有生成的动态库
- **可执行文件**：记录所有生成的可执行程序
...
**文件位置：**
- 默认特性：`.c2rust/default/c/targets.list`
- 自定义特性：`.c2rust/<feature>/c/targets.list`

**用途：**
这个文件帮助用户了解构建过程中生成了哪些二进制文件，便于在 C 到 Rust 转换过程中选择正确的翻译目标。用户可以基于这个列表来决定哪些库或程序需要被转换为 Rust 代码。
