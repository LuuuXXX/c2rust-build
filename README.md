...
- **交互式文件选择**：提供用户友好的树形界面选择需要翻译的预处理文件
  - 按目录结构分层显示文件（树形视图）
  - 支持选择整个文件夹（自动包含文件夹内所有文件）
  - 可视化层级关系，便于批量选择相关文件
  - **自动清理**：确认选择后，未被选中的预处理文件会被自动删除；若删除后某个目录变为空目录，该目录也会被一并删除
- **目标制品选择**：在文件选择后，交互式选择目标制品（可执行程序、静态库或动态库）
...
2. **新的 libhook.so 在构建过程中直接生成预处理文件和 targets.list**到 `.c2rust/<feature>/c/` 目录
3. **读取目标制品列表**（由 libhook.so 生成的 targets.list），并提供交互式界面让用户选择目标制品（可执行程序、静态库或动态库）
4. 收集生成的预处理文件，并提供交互式界面让用户选择**参与构建此 target 的文件**
5. 将用户选择保存到 `.c2rust/<feature>/selected_files.json`，并**删除未被选中的预处理文件**；若删除后某目录变为空目录，该目录也会被一并删除
6. 将构建配置和选择的目标制品保存到项目配置
7. **自动保存**当前命令执行目录（相对于 `.c2rust` 文件夹所在目录）
8. **自动提交**（如果存在 `.c2rust/.git`）：将所有修改提交到本地 git 仓库
...
在构建过程完成后，工具会首先自动读取 `targets.list` 并提供交互式界面供您选择目标制品：
...
选择的目标制品会通过 `c2rust-config` 保存到配置中，供后续构建步骤使用。

**非交互模式**：如果使用 `--no-interactive` 参数或在非 TTY 环境中运行，工具会自动选择第一个目标制品。

**无目标制品时**：如果 `targets.list` 为空或不存在，工具会显示警告并跳过目标选择步骤，继续执行后续流程。

#### 文件选择

在目标选择完成后，工具会自动收集生成的预处理文件，并提供交互式界面供您选择**参与构建此 target 的文件或文件夹**：
...
**文件夹选择功能（层级选择）**：
- **选择文件夹（📁）**：勾选一个文件夹后，在**最终确认选择时**，该文件夹内的**所有文件**（包括子目录中的文件）都会被视为选中并参与后续流程（界面中的子项勾选状态不会实时自动联动）
- **取消选择文件夹（📁）**：取消勾选一个文件夹后，在**最终确认选择时**，该文件夹内的**所有文件**（包括子目录中的文件）都会被视为未选中并从后续流程中排除
- 支持递归层级：在你确认选择后，对父文件夹的包含/排除会递归作用到所有子文件夹及其文件，而不仅仅是当前层级
- 方便批量选择或排除整个模块的文件
...
**使用技巧**：
- 💡 **批量选择模块**：选择 `src/` 文件夹可一次性选中整个 src 模块的所有文件
- 💡 **排除某个目录**：取消选择不需要的文件夹（如 `tests/`），该目录下所有文件将被自动排除
- 💡 **子目录控制**：当前规则为「父目录优先」：在选中父文件夹时，无法仅通过取消子文件夹来排除其内容。如需排除某个子目录，可先取消选中父文件夹，再只选择需要保留的子目录或文件

选择的文件列表会保存到 `.c2rust/<feature>/selected_files.json`，供后续翻译步骤使用。

**确认选择后的自动清理**：
- **删除未选中的预处理文件**：所有未被选中的预处理文件（`.c2rust`、`.i`、`.ii`）将从磁盘中删除
- **清理空目录**：若删除文件后某个目录变为空目录，该目录也会被自动递归删除（清理范围不超过 `.c2rust/<feature>/c/` 边界）
- **安全机制**：如果未选择任何文件（空选择），则不执行清理操作，以防止意外删除所有文件
...
4. **构建追踪**：使用 LD_PRELOAD 钩子库在追踪编译器调用的同时执行构建命令
   ...
   - **新的 libhook.so 直接生成预处理文件和 targets.list**到 `.c2rust/<feature>/c/` 目录
5. **目标制品选择**：
   - 读取由 libhook.so 生成的 `.c2rust/<feature>/c/targets.list` 文件
   - 显示交互式目标制品选择界面
   - 用户选择一个目标制品（可执行程序、静态库或动态库）
   - 将选择通过 `c2rust-config` 保存到配置中
   - 如果没有可用的目标制品，跳过此步骤
6. **文件收集与选择**：
   - 遍历 `.c2rust/<feature>/c/` 目录收集所有生成的预处理文件
   - 显示交互式文件选择界面，提示用户选择**参与构建此 target 的文件**
   - 用户可以选择需要翻译的文件（默认不选中任何文件，请按需勾选）
   - 将选择保存到 `.c2rust/<feature>/selected_files.json`
   - **删除未选中的预处理文件**：确认选择后，所有未被勾选的预处理文件将从磁盘删除
   - **清理空目录**：若删除预处理文件后某个目录变为空目录，该目录也会被自动删除（递归向上清理，但不超过 `.c2rust/<feature>/c/` 边界）
7. **配置保存**：通过 `c2rust-config` 保存构建配置：
   - `build.dir`：构建目录（自动检测，相对于项目根目录）
   - `build.cmd`：完整的构建命令字符串
   - `target`：选择的目标制品路径（如果已选择）
   - 配置可以关联到特定的特性（通过 `--feature` 参数）
8. **自动提交**（可选）：如果 `.c2rust` 目录下存在 git 仓库（`.c2rust/.git`），工具会自动提交所有修改：
...
`targets.list` 文件由 **libhook.so 在构建过程中直接生成**，记录所有链接的二进制文件。

**文件位置**：`.c2rust/<feature>/c/targets.list`

**包含的二进制类型**：
- **静态库**：以 `lib` 开头且以 `.a` 结尾的静态库文件（遵循 Unix/Linux 命名约定）
- **动态库**：共享库文件（`.so` 文件或带版本号的 `.so.1`、`.so.2` 等）
- **可执行程序**：构建的可执行二进制文件
...
每个条目是二进制文件的**basename**（文件名），每行一个。libhook.so 在链接时记录输出文件名。
...
**注意事项**：
- `targets.list` 由 **libhook.so 在链接阶段直接生成**，无需额外扫描
- 只包含最终的二进制产物的文件名（basename），不包含路径信息
- 通过 LD_PRELOAD 机制在链接器调用时自动记录
- 文件在每次构建前会被清空，避免旧构建的条目残留
- 如果多个目录有同名二进制文件，它们会显示为同一个条目
