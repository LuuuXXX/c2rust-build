...
3. **生成 targets.list 文件**，包含所有构建的二进制文件（静态库、动态库、可执行程序）
4. **提供目标制品选择**，让用户选择 Rust 代码归属的目标制品
5. 收集生成的预处理文件，并提供交互式界面让用户选择需要翻译的文件
6. 将用户选择保存到 `.c2rust/<feature>/selected_files.json`
7. 将构建配置和选择的目标保存到项目配置
8. **自动保存**当前命令执行目录（相对于 `.c2rust` 文件夹所在目录）
9. **自动提交**（如果存在 `.c2rust/.git`）：将所有修改提交到本地 git 仓库
...
在构建完成并生成 `targets.list` 文件后，工具会提示您选择一个目标制品（静态库、动态库或可执行程序），Rust 翻译的代码将归属于该目标：

```
=== Target Selection ===
Available target artifacts:
  1. libmath.a
  2. myapp

Select a target artifact to translate:
  - Enter the number of the target

Your selection: 1
Selected target: libmath.a
✓ Stored in config: build.target = libmath.a
```

**关键特性：**
- 自动从 `targets.list` 读取可用的目标制品
- 按照写入顺序显示，自动去重
- 如果只有一个目标，自动选择
- 在非交互模式下（使用 `--no-interactive` 标志），自动选择第一个目标
- 选择的目标保存到 `config.toml` 的 `build.target` 字段

**注意事项：**
- 如果存在多个制品需要翻译，应该分开完成（每次选择一个目标）
- 选择的目标决定了翻译代码在混合构建时的作用范围

#### 文件选择

在目标选择后，工具会自动收集生成的预处理文件，并提供交互式界面供您选择：
...
4. **构建追踪**：使用 LD_PRELOAD 钩子库在追踪编译器调用的同时执行构建命令
   ...
   - **生成 targets.list 文件**记录所有构建的二进制目标
5. **二进制目标处理**：
   - 扫描项目目录发现所有二进制文件（静态库、动态库、可执行程序）
   - 生成去重的 `targets.list` 文件
   - 每次构建都会刷新 targets.list 以确保准确性
6. **目标制品选择**：
   - 读取 `targets.list` 文件获取可用的目标制品
   - 在交互模式下提示用户选择目标制品
   - 在非交互模式下自动选择第一个目标
   - 如果只有一个目标，自动选择
   - 将选择的目标保存到配置文件的 `build.target` 字段
7. **文件收集与选择**：
   - 遍历 `.c2rust/<feature>/c/` 目录收集所有生成的预处理文件
   - 显示交互式文件选择界面
   - 用户可以选择需要翻译的文件（默认全选）
   - 将选择保存到 `.c2rust/<feature>/selected_files.json`
8. **配置保存**：通过 `c2rust-config` 保存构建配置：
   - `build.dir`：构建目录（自动检测，相对于项目根目录）
   - `build.cmd`：完整的构建命令字符串
   - `build.target`：用户选择的目标制品
   - 配置可以关联到特定的特性（通过 `--feature` 参数）
9. **自动提交**（可选）：如果 `.c2rust` 目录下存在 git 仓库（`.c2rust/.git`），工具会自动提交所有修改：
...
- **可执行程序**：构建的可执行二进制文件（有执行权限且不是脚本或源文件）
...
**用途**：
- 追踪项目中所有的二进制产物
- **提供目标制品选择**：用户从列表中选择 Rust 代码归属的目标
- 便于后续处理和 Rust 混合构建
- 帮助确定哪些库需要与转换后的 Rust 代码集成

**目标制品选择**：
- 在文件选择之前，工具会提示用户从 `targets.list` 中选择一个目标制品
- 选择的目标决定了翻译代码在混合构建时的作用范围
- 如果选择的是静态库，Rust 静态库总是和被选择的静态库一起使用
- 如果选择的是动态库或可执行程序，Rust 静态库只在该目标构建时使用
- 选择结果保存到配置文件的 `build.target` 字段
...
- 如果存在多个制品需要翻译，应该分开完成（每次构建选择一个目标）
